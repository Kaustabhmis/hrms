...
with checks as (
    ...
    union all
    select 10, 'Admin functions reachable without signing in',
           -- anon exists only on Supabase; on a plain PostgreSQL there is no
           -- web API to be reachable from, so the answer there is zero.
    ...
    union all
    select 11, 'Companies set up so far',
           (select count(*)::text from companies), 'any'
    union all
    select 12, 'Accounts set up so far',
